...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      if(!image){
        toast.error('Thumbanail Not Selected')
      }
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-beteen md:p-8 md:pb-0 p-4 pt-8 pb-0'>
       <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input onChange={ e => setCourseTitle(e.target.value)} value={courseTitle}  type="text" placeholder='Type Here' className='outline-none md:py-2.5 rounded  py-2 px-3 border border-gray-500' required />
          </div>
          {/* -------- course description input ------------ */}
          <div className='flex flex-col gap-1'>
            <p className='text-semibold'>Course Description</p>
            <div ref={editorRef}></div>
          </div>

          {/* -------- course thumbnail and price input ------------ */}
          <div className=' flex items-center justify-between flex-wrap '>
             {/* -------- course price input ------------ */}
            <div className='flex flex-col gap-1'>
              <p>Course Price</p>
              <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice} type="number" className='outline-none md;py-2.5 py-2 w-28 rounded border border-gray-500' required />
            </div>
              ...
              <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
                ...
                {!chapter.collapsed && (
                  <div className='p-4'>
                    ...
                    <div className='inline-flex bg-gray bg-gray-100 p-2 rounded cursor-pointer mt-2' onClick={()=>handleLecture('add', chapter.chapterId)}>+ Add Lectures</div>
                  </div>
                )}
              </div>
            ))}
